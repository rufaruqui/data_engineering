...
\begin{frame}{The Hadoop Ecosystem: Layered Architecture}
  ...
  \begin{alertblock}{Design Philosophy}
   \footnotesize Each layer is \textbf{independently replaceable}.  YARN replaced MapReduce's resource manager in Hadoop 2.0, allowing Spark, Tez, and Storm to share the same HDFS cluster without rewriting data storage.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ecosystem Components: Roles at a Glance}
  ...
  \begin{tabular}{p{1.8cm}p{1.5cm}p{9.5cm}}
    \toprule
...
\begin{frame}{NameNode: The Brain of HDFS}
      ...
      \begin{exampleblock}{NameNode Metadata Persistence}
        ...
        \begin{tikzpicture}[
          ...
          \node[box, fill=PUNavy!20] (fs)  at (-3.5, 2.8) {\textbf{FsImage}\\{\tiny filesystem snapshot on disk}};
          \node[box, fill=PUOrange!20] (el) at (2, 2.8) {\textbf{Edit Log}\\{\tiny all changes since snapshot}};
          \node[box, fill=PUNavy!70, text=white] (nn) at (0, 1.7)
...
\begin{frame}{DataNode: Block Storage in Practice}
      ...
      \begin{exampleblock}{Block Storage: How a 300 MB File is Stored}
        ...
        \begin{tikzpicture}[
          ...
          \node[blk, fill=PUNavy!20]  (b1) at (-3.5,2.5) {Block 1 (128 MB)};
          \node[blk, fill=PUNavy!30]  (b2) at ( 0.5,2.5) {Block 2 (128 MB)};
          \node[blk, fill=PUNavy!40]  (b3) at ( 3.5,2.5) {Block 3 (44 MB)};
...
\begin{frame}{HDFS Write Path: Pipeline Replication}
      ...
      \begin{tikzpicture}[
        ...
        \node[actor, fill=PUOrange!70, text=white] (cl) at (-2.0,   6.5) {Client};
        \node[actor, fill=PUNavy,       text=white] (nn) at (6.0, 6.5) {NameNode};
        \node[actor, fill=PUTeal!80,    text=white] (d1) at (-2.0,   2.8) {DN1};
        \node[actor, fill=PUTeal!60,    text=white] (d2) at (2.0, 2.8) {DN2};
        \node[actor, fill=PUTeal!40,    text=white] (d3) at (6.0, 2.8) {DN3};
        ...
        % Acks back
        \draw[ack] (d3) to[bend right=35]
              node[below, font=\tiny]{\textcircled{6} ack} (d2);
        \draw[ack] (d2) to[bend right=35]
              node[below, font=\tiny]{ack} (d1);
        \draw[ack] (d1) to[bend right=35]
...
\begin{frame}{Textbook \& Reading References}
  ...
  \end{block}
  \vspace{6pt}
  \begin{exampleblock}{Supplementary (Covered in Week 3, Part 2 — Research Article)}
    \begin{itemize}
      \small
...
\begin{frame}[c]
  \begin{center}
    ...
    \begin{block}{\centering Next Lecture: Week 3, Part 2}
      \centering
      Research Article: Ghemawat, Gobioff \& Leung (2003) — \textit{``The Google File System''}\\[4pt]
