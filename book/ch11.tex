...
TABLE_PATH = "hdfs:///lakehouse/bkash/transactions"
...
schema = StructType([
    ...
    StructField("amount_bdt", DoubleType(),    True),
    StructField("ts",         TimestampType(), True),
    StructField("district",   StringType(),    True),
])

df_initial = spark.createDataFrame([
    ("T001", "01711000001", "01711000002", 500.0,  None, "Dhaka"),
    ("T002", "01711000003", "01711000004", 1200.0, None, "Chittagong"),
], schema=schema)
...
df_new = spark.createDataFrame([
    ("T003", "01711000005", "01711000001", 250.0, None, "Dhaka"),
    ("T004", "01711000006", "01711000007", 750.0, None, "Sylhet"),
], schema=schema)
...
spark.sql(f"""
    ...
    SET    amount_bdt = amount_bdt * 1.10
    WHERE  txn_id = 'T001'
""")
...
\begin{enumerate}
  ...
  \item \textbf{Federated computational governance}: global policies
        (GDPR compliance, PDPA data localisation, HIPAA de-identification,
...
mlflow.set_experiment("bkash-fraud-detection-v2")
...
feature_cols = ["amount_bdt", "txn_per_hour",
...
with mlflow.start_run(run_name="gbt_maxDepth5_lr01"):
    ...
    mlflow.spark.log_model(model, "gbt_fraud_model",
                           registered_model_name="bkash-fraud-gbt")

# --- Transition to Staging via Model Registry API ---
client = mlflow.MlflowClient()
latest = client.get_latest_versions("bkash-fraud-gbt",
                                     stages=["None"])[0]
client.transition_model_version_stage(
    name    = "bkash-fraud-gbt",
...
# True dengue case counts by district (confidential)
true_counts = {
    "Dhaka":       1_402,
    "Chittagong":  387,
    "Rajshahi":    201,
...
print(f"ε = {epsilon} (stronger privacy → larger noise)")
print(f"{'District':<14} {'True':>8} {'Published':>12} {'Error':>8}")
print("-" * 46)

for district, count in true_counts.items():
    published = laplace_mechanism(count, sens, epsilon, rng)
    error     = abs(published - count)
    print(f"{district:<14} {count:>8} {published:>12.1f} {error:>8.1f}")

# Compare: tighter budget → more noise
for eps in [0.1, 0.5, 1.0, 2.0, 5.0]:
    noised = laplace_mechanism(1_402, 1.0, eps, rng)
    print(f"  ε={eps:<4}: published Dhaka count = {noised:.1f}")
\end{lstlisting}
...
def extract_key_amount(record: dict):
    """Emit (district, amount_bdt) pairs."""
    return (record.get("district", "Unknown"),
            float(record.get("amount_bdt", 0.0)))
...
def format_output(kv) -> str:
    district, total = kv
    return f"{district}: BDT {total:,.0f}"
...
with beam.Pipeline(options=options) as p:
    result = (
        ...
        # WHAT: aggregate amount per district per window
        | "SumByDistrict"  >> beam.CombinePerKey(SumValues())
...
A bKash transaction table is stored as a Delta table on S3. Three
...
\begin{itemize}
  ...
  \item Job B: UPDATE 500 transactions where \texttt{district='Dhaka'}
        (started 2 seconds after Job A).
  \item Job C: DELETE all transactions where \texttt{amount\_bdt < 10}
...
A DGHS team built a sepsis prediction model in 2022. It is deployed in
production at Dhaka Medical College Hospital and makes predictions every
15 minutes for each ICU patient. In 2025, the team notices the model's
...
Design a Beam pipeline for a real-time Pathao driver location analytics
system:
\begin{itemize}
  ...
  \item Required output: count of active drivers per district per
...
\begin{enumerate}[(a)]
  ...
  \item \textbf{How}: specify the accumulation mode. If the 30-second
        early trigger emits ``Dhaka: 12{,}400 drivers'' and the final
        watermark trigger emits ``Dhaka: 12{,}800 drivers'', what does the
...
The DGHS plans to publish monthly district-level disease statistics
(dengue case counts, cholera hospitalisations, COVID-19 test positivity
rate) for 64 districts. Each statistic is a COUNT or AVERAGE query over
the patient records database. The total monthly privacy budget is
$\varepsilon_{\text{total}} = 2.0$.
\begin{enumerate}[(a)]
  \item The DGHS wants to publish 5 statistics per district per month
        (dengue count, cholera count, COVID count, COVID positivity rate,
        and flu count). How should the budget be allocated across these
        5 queries using \emph{sequential composition} (each query spends
        from the budget independently)?
  \item A statistician proposes using \emph{parallel composition} instead:
        since the district-level counts are disjoint subsets of the total
        population, the budget does not need to be divided across districts.
        Explain why this is correct and what it implies for the per-district
        noise level.
  \item With $\varepsilon = 0.4$ per query (from part (a)), and a COUNT
        query sensitivity of $\Delta f = 1$, compute the standard deviation
        of the Laplace noise. Is this noise level acceptable for a district
        with 1{,}400 dengue cases? For a district with 14 cases?
...
mobile payment company in Bangladesh (bKash scale: 50 million transactions/day,
...
\begin{enumerate}[(a)]
  ...
  \item \textbf{Storage design}: design the Delta table schema for the
        ...
        district-level aggregations for dashboards).
...
\dpr \textbf{Bangladesh National Health Lakehouse.}
Design a Lakehouse for the DGHS national health analytics platform,
integrating data from 300 hospitals (HL7 FHIR), pharmacy chains,
and community health workers (mobile app events via Kafka).
...
\begin{enumerate}[(a)]
  ...
  \item \textbf{Privacy}: specify the de-identification pipeline that
        ...
        Which safe harbour identifiers are removed, and which are
        generalised (e.g., date-of-birth $\to$ age-band)?
  \item \textbf{Federated learning}: the DGHS wants to train a sepsis
        prediction model without ever centralising individual patient
        records from private hospitals. Design the federated learning
        architecture: number of rounds, local epochs per round, FedAvg
        aggregation, and validation strategy.
  \item \textbf{Real-time alerting}: specify the Dataflow pipeline for
        detecting a cholera outbreak: window type and size, trigger,
...
\begin{enumerate}[(a)]
  \item Create a Delta table for DGHS hospital admissions:
        \texttt{(patient\_id STRING, district STRING, diagnosis\_code STRING,
...
\begin{enumerate}[(a)]
  ...
  \item Register the best model in the MLflow Model Registry with the
        name \texttt{bkash-fraud-v2}. Transition it to the
        \texttt{Staging} stage.
...
Implement an Apache Beam pipeline that aggregates bKash transaction events:
\begin{enumerate}[(a)]
  \item Generate a synthetic stream of 1 million transaction events with
        \texttt{(txn\_id, district, amount\_bdt, event\_time)}. Introduce
  ...
  \item Write the pipeline using the Direct Runner (local execution).
        ...
        \texttt{amount\_bdt} per \texttt{district} per window.
