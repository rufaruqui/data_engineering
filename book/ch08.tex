...
\begin{enumerate}
  \item \textbf{Topic.} A \term{topic} is a named, durable, append-only log
        ...
        but append-only: \texttt{payment-events}, \texttt{rideshare-gps-pings},
        and \texttt{city-weather-sensors} might each be a Kafka topic.
...
\begin{tikzpicture}[
  ...
  \node[font=\normalsize\bfseries\sffamily, text=DEBlue] at (3.6, 6.4)
        {Topic: \texttt{payment-txns}\ \ (3 partitions, RF\,=\,2)};
  ...
  %% Consumer Group
  \node[font=\small\bfseries\sffamily, align=center] at (9.4, 5.2) {Consumer\\Group};
...
\begin{enumerate}
  ...
  \item \textbf{Regulatory requirement mandates independent batch
        reconciliation.} EU PSD2 Article~94 and equivalent national
        payment regulations require that all electronic payment
        transactions be reconciled in a separate batch system to detect
        discrepancies between the real-time ledger and the daily
        settlement---a process that cannot be replaced by a streaming job.
\end{enumerate}
\end{caution}
...
\begin{itemize}
  ...
  \item \textbf{Privacy leakage.} Personally identifiable information
        ...
        the governance layer. Under GDPR Article~83(5) and equivalent
        data-protection frameworks worldwide, this can result in
        significant regulatory penalties and mandatory breach notification.
...
A mobile payment company expects the following workload
...
An international commercial bank is designing a data pipeline for three
workloads:
\begin{enumerate}
  ...
  \item Monthly regulatory capital report: aggregates 36 months of
        transaction data into statutory metrics as required by
        Basel~III / EU CRR capital adequacy reporting rules.
...
Design the zone structure for a regional healthcare data lake that will
store:
\begin{itemize}
  \item Patient records from 80 regional hospitals (structured, RDBMS)
...
sensitive health data under HIPAA (US) or GDPR Article~9 (EU).

\exercisesection{Design Problems}

\dpr \textbf{Mobile Money Real-Time Fraud Pipeline.}
Design a complete Kafka-based data pipeline for a mobile financial service
(e.g., M-Pesa Kenya, 30+ million users, 10 million transactions/day)
that must:
\begin{enumerate}[label=(\alph*)]
  ...
  \item Power a live operations dashboard showing total transaction
        ...
        seconds.
  \item Provide a daily reconciliation file to the financial regulator
        containing all flagged transactions and final dispositions
        (confirmed fraud vs.\ false positive).
...
A mid-size university wishes to build a data lake to unify its operational
data for academic analytics, student counselling, and research reporting.
Data sources include: student RDBMS (Oracle, 50K student records), LMS
(Moodle, JSON event logs), library system (MySQL, book borrowing records),
exam results (Excel uploads by faculty), and Wi-Fi access logs (syslog,
100K events/day).
\begin{enumerate}[label=(\alph*)]
  ...
  \item Identify which datasets contain PII under FERPA (US) or GDPR
        (EU) and describe the access control policies (who can see what,
        in what form) to be enforced by Apache Ranger.
\end{enumerate}
...
\begin{enumerate}[label=(\alph*)]
  \item Implement a producer that generates 50{,}000 synthetic mobile
        ...
        \texttt{receiver}, \texttt{amount\_usd}, \texttt{timestamp\_ms}).
  ...
  \item Implement a consumer in group \texttt{analytics-v1} that reads
        all messages and computes: (i) total transaction volume in USD;
...
COUNTRY_CODES = ["+1", "+44", "+49", "+33", "+81", "+61"]
SENDERS = [f"{random.choice(COUNTRY_CODES)}{random.randint(2000000000,9999999999)}"
           for _ in range(1000)]
...
for _ in range(50_000):
    ...
    event = {
        ...
        "amount_usd":   round(random.uniform(1.00, 5_000.00), 2),
...
Simulate a Lambda architecture for a ride-sharing app (e.g., Uber or Grab)
...
\begin{enumerate}[label=(\alph*)]
  \item \textbf{Batch layer:} Read a 1-million-row Parquet file of
        ...
        \texttt{driver\_id}, \texttt{pickup\_zone}, \texttt{fare\_usd},
...
# Schema: pickup_zone, hour_ts, total_fare_usd, source='batch'
...
live_events = spark.createDataFrame([
    {
        ...
        "pickup_zone":  random.choice(["Downtown","Midtown","Airport",
                                       "Suburbs","University","Harbour"]),
        "fare_usd":     round(random.uniform(5.00, 80.00), 2),
...
real_time_view = (live_events
    .groupBy("pickup_zone")
    .agg(_sum("fare_usd").alias("total_fare_usd"))
...
\begin{enumerate}[label=(\alph*)]
  \item Load a CSV file of customer records (fields: \texttt{customer\_id},
        \texttt{mobile}, \texttt{name}, \texttt{region}, \texttt{balance\_usd},
  ...
  \item Using the \texttt{great\_expectations} library, define and run
        ...
        \begin{itemize}
          ...
          \item \texttt{mobile} must match the E.164 international format
                regex \texttt{\textasciicircum{}\textbackslash +[1-9]\textbackslash
                d\{7,14\}\$}.
          \item \texttt{balance\_usd} must be between 0 and 10{,}000{,}000.
          \item \texttt{region} must be one of the values in a reference
                catalogue (e.g., ISO~3166-2 region codes for the country
                of operation).
