...
\begin{enumerate}
  ...
  \item Explain three privacy-preserving techniques for healthcare analytics
        ...
        (HIPAA, GDPR, or Bangladesh PDPA) mandates each.
\end{enumerate}
\end{chapterlearning}
...
views. In Bangladesh, Facebook is the dominant social platform with
50 million users---making it the primary channel for crisis communication,
political discourse, and commercial marketing. The data they generate is
...
\begin{enumerate}
  ...
  \item \textbf{Irregularity (Holiday Effects)}: one-off events
        (Eid, national elections, product launches, natural disasters)
        cause sharp deviations from the seasonal pattern. These cannot
        be captured by a Fourier seasonality model and must be encoded
        explicitly.
\end{enumerate}
\end{defbox}
...
\begin{itemize}
  ...
  \item \textbf{Holiday effects} $h(t)$: a user-specified table of
        one-off events (Eid, Puja, national elections, cricket World Cup
        finals) is encoded as indicator variables that absorb the effect
        of each event. Events with multi-day impact (the three days
        around Eid) are represented with a window.
...
\begin{lstlisting}[style=pyspark, caption={Prophet on one time series:
  bKash daily transaction volume with Eid holiday effect}]
import pandas as pd
import numpy as np
...
# Simulate 3 years of daily bKash transaction volume
...
# Define Eid holidays (bKash volume spikes during Eid shopping)
holidays = pd.DataFrame({
    "holiday":      "Eid",
    "ds":           pd.to_datetime([
                        "2021-05-13", "2021-07-21",
                        "2022-05-03", "2022-07-10",
                        "2023-04-22", "2023-06-29",
                    ]),
    "lower_window": -2,   # 2 days before Eid
    "upper_window":  3,   # 3 days after Eid
})
...
fig1.axes[0].set_title("bKash Daily Transaction Volume  --  Prophet Forecast")
fig1.savefig("bkash_forecast.png", dpi=150, bbox_inches="tight")

fig2 = m.plot_components(forecast)
fig2.savefig("bkash_components.png", dpi=150, bbox_inches="tight")
print("Saved: bkash_forecast.png and bkash_components.png")
\end{lstlisting}
...
# Example: bKash agent hourly transaction counts
df = spark.read.parquet("hdfs:///data/bkash/agent_hourly_txn/")
...
2--5~GB of new EHR data per day. At the national level, Bangladesh's
DGHS has 15+ years of digitised records across national hospitals,
representing sufficient scale for population-level clinical analytics.
...
Bangladesh's DGHS national hospital network has $\sim$2 million
digitised inpatient encounters per year across tertiary hospitals---if
integrated, sufficient for mortality and readmission modelling.
\end{keyinsight}

%--------------------------------------------------------------------
\subsection{Privacy: HIPAA, GDPR, and the Bangladesh PDPA}
...
\begin{itemize}
  ...
  \item \textbf{Bangladesh PDPA} — Personal Data Protection Act (under
        development as of 2025): based on GDPR principles. National
        Identification (NID) card numbers are uniquely identifying across
        the population; records linked to NID numbers require explicit
        consent under the draft act. Health records submitted to national
        health information systems (DGHS) are governed by the Health
        Data Policy of 2021, which mandates encryption at rest and in
        transit for all patient-identifiable data.
\end{itemize}
\end{defbox}
...
wearables, DICOM imaging), three privacy frameworks (HIPAA, GDPR, PDPA),
...
time series (e.g., daily bKash transactions) where strong autocorrelation
would be expected.
...
A Facebook group for a DGHS dengue alert campaign has 1 million users.
...
A Prophet model was fitted to 3 years of weekly DGHS dengue case data
for Dhaka. The fitted components show:
\begin{itemize}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \item The DGHS director asks whether the model can predict a dengue
        surge 8 weeks in advance. Describe the specific uncertainty that
...
A DGHS researcher wants to analyse 5 years of inpatient records
(500{,}000 patients) from 10 national hospitals to build a 30-day
readmission prediction model. The data includes: patient name, NID
number, date of birth, home district, ICD-10 diagnosis codes, procedure
codes, admission date, length of stay, and discharge outcome.
\begin{enumerate}[label=(\alph*)]
  \item List all fields that would be removed under HIPAA's safe harbour
        method. Which field in the Bangladesh context is most uniquely
        identifying and why?
  ...
  \item A second researcher at BUET wants to train on their own 50{,}000-patient
        dataset and combine insights with the DGHS model. Why would
        federated learning be preferable to data centralisation in this
        scenario? Describe the federated averaging protocol and identify
        one risk.
  \item The final deployed model has AUROC 0.82. The DGHS Chief Data
        Officer asks: ``Is the model equally accurate for patients from
        rural districts as for patients from Dhaka?'' Describe the
...
A Bangladeshi telecom company needs to forecast daily call-drop rates
for 6{,}000 cell towers, each with 2 years of daily measurements.
\begin{enumerate}[label=(\alph*)]
...
\dpr \textbf{Bangladesh Disease Surveillance System.}
Design a big data disease surveillance system for Bangladesh analogous
to the CDC BioSense platform. The system should detect outbreaks of
dengue fever, influenza, cholera, and COVID-19 up to 7 days before
the DGHS receives clinical confirmation.

\begin{enumerate}[label=(\alph*)]
  \item \textbf{Data sources}: identify four non-traditional data sources
        analogous to the CDC's syndromic surveillance inputs (social media,
        pharmacy, ER, search), specific to Bangladesh. For each, identify
        the entity that controls the data and the access mechanism.
  ...
  \item \textbf{Signal processing}: describe the Spark Structured Streaming
        ...
        weekly DGHS hospitalization series.
  ...
  \item \textbf{Privacy safeguards}: the social media signal includes
        ...
        these signals enter the pipeline.
\end{enumerate}

\dpr \textbf{Multi-Hospital Clinical ML Pipeline.}
Chittagong Medical College Hospital (CMCH) wants to deploy a
30-day readmission prediction model trained on data from five hospitals
in the Chittagong division (CMCH, general hospitals in Cox's Bazar,
Comilla, Noakhali, and Feni). Data sharing between hospitals is
restricted by Bangladesh PDPA.
...
\begin{enumerate}[label=(\alph*)]
  ...
  \item The central model achieves AUROC 0.79 after 10 federated rounds.
        Hospital A's local model (CMCH, 50{,}000 patients) achieves AUROC
        0.84. The hospital director asks whether to use the global model
        or the local model. Analyse the trade-offs and provide a
        recommendation.
...
\pe \textbf{Prophet Forecasting with Eid Holiday Effects.}
Using the Prophet library, build a demand forecasting model for a
Bangladeshi retailer with strong Eid seasonality.
\begin{enumerate}[label=(\alph*)]
  \item Generate 3 years of synthetic weekly sales data with: an upward
        linear trend, strong annual seasonality (peak at Eid-ul-Fitr in
        May, secondary peak at Eid-ul-Adha in July), moderate weekly
        seasonality (peak on Friday), and random Gaussian noise.
  \item Fit two Prophet models: one without holidays and one with the
        Eid dates for the 3 years explicitly encoded with a window of
        $[-3, +3]$ days. Compare the MAPE on a 12-week holdout set.
...
\begin{enumerate}[label=(\alph*)]
  \item Generate 100{,}000 synthetic patient records with fields:
        ...
        \texttt{district} (64 values), \texttt{primary\_icd10} (50 codes),
  ...
  \item Apply safe harbour de-identification: generate a surrogate
        ...
        and group \texttt{district} into 8 divisions.
  ...
  \item Stratify the AUROC by district. Identify the three districts
        with the lowest AUROC and hypothesise (in 2--3 sentences)
        why the model performs worse in those districts.
\end{enumerate}
