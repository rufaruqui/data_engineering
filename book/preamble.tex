...
\usepackage[expansion=false]{microtype} % kerning/tracking only; expansion off (font compat)
\usepackage{setspace}
...
%----------------------------------------------------------------------
\usepackage{amsmath}
% newpxmath defines \Bbbk; amssymb would redefine it — clear first
\let\Bbbk\relax
\usepackage{amssymb}
% newpxmath also defines \openbox; clear before amsthm
\let\openbox\relax
\usepackage{amsthm}
...
% Global listing defaults — handle UTF-8 special chars that appear in comments
\lstset{
  extendedchars=true,
  inputencoding=utf8,
  literate=
    {—}{{---}}{1}%  em dash  → three hyphens (typewriter convention)
    {–}{{--}}{1}%   en dash  → two hyphens
    {…}{{...}}{1}%  ellipsis → three dots
    {←}{{\texttt{<-}}}{2}%
    {→}{{\texttt{->}}}{2}%
}
...
\usepackage{pdfpages}   % for \includepdf{cover.pdf}
