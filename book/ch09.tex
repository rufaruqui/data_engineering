...
# Simulate 5 million GPS coordinates around New York City
x = rng.normal(-73.95, 0.15, 5_000_000)   # longitude
y = rng.normal(40.75,  0.10, 5_000_000)   # latitude
...
plt.tight_layout()
plt.savefig("nyc_gps_hexbin.png", dpi=150, bbox_inches="tight")
print("Saved nyc_gps_hexbin.png")
\end{lstlisting}
...
x = rng.normal(-73.95, 0.12, 200_000)
y = rng.normal(40.75,  0.08, 200_000)
...
ax.set_title("KDE Density Contour  --  NYC GPS Pings")
ax.set_xlabel("Longitude")
ax.set_ylabel("Latitude")
plt.tight_layout()
plt.savefig("nyc_kde.png", dpi=150)
\end{lstlisting}
...
(``show Q1 2024, North-East region only'') to respond in under one second.
...
region, and product category---the data cube pre-computes eight
...
cube_df = (fact
    .cube("txn_date", "region", "product_category")
    .agg(
        _sum("amount_usd").alias("total_usd"),
...
region_q1 = spark.read.parquet("hdfs:///data/curated/txn_cube/") \
    .filter(
        (col("txn_date").between("2024-01-01", "2024-03-31")) &
        col("region").isNotNull() &
        col("product_category").isNull()
    ) \
    .orderBy("total_usd", ascending=False)
...
\begin{itemize}
  \item An operations dashboard showing the number of active Uber
        drivers by city zone, updated every 5 seconds.
  \item A payment fraud operations centre showing transaction volume
        per minute and active alert count.
  \item A CDC / NHS disease surveillance dashboard showing new case
        reports as hospitals submit their hourly returns.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Spark Structured Streaming or Flink}: subscribes to
        ...
        (e.g., count of transactions per minute per region).
...
\begin{tikzpicture}[
  ...
  \node[box, fill=DEGray!20] (prod) at (0, 6.2)
        {Event Producers\\{\small PayPal · Uber · IoT}};
...
const data = [
  { region: "New York",   cases: 4217 },
  { region: "London",     cases: 1803 },
  { region: "Berlin",     cases:  892 },
  { region: "Sydney",     cases:  671 },
  { region: "Toronto",    cases:  544 },
];
...
// Minimal y-axis: region names only
...
svg.selectAll(".bar")
   .on("mouseover", (event, d) => {
     tooltip.style("opacity", 1)
            .html(`<b>${d.region}</b><br>Cases: ${d.cases.toLocaleString()}`);
   })
   .on("mousemove", event => {
...
region, category, and channel---how many cuboids does the full data
cube contain?
...
A ride-sharing company data scientist needs to build an operations dashboard
for the city fleet management team showing:
\begin{itemize}
  \item City-wide active driver count by zone (20 city zones)
...
A mobile payment fraud operations dashboard must update its fraud alert
rate chart within 10 seconds of an event arriving at the source system.
...
\texttt{hour\_ts} (43{,}800 hours over 5 years), \texttt{region}
(50 regions), and \texttt{tariff\_band} (5 bands).
\begin{enumerate}[label=(\alph*)]
  ...
  \item Design the materialised cuboids for a dashboard that must serve:
        (i) hourly consumption per region for any 30-day window;
        (ii) monthly total consumption per tariff band; (iii) peak-hour
        demand by region.
  \item Estimate the storage size (in GB) of the hourly-by-region
        cuboid assuming 30 bytes per row.
\end{enumerate}

\exercisesection{Design Problems}

\dpr \textbf{Public Health Surveillance Dashboard.}
A national public health agency (e.g., CDC or NHS) needs a disease
surveillance dashboard communicating:
\begin{enumerate}[label=(\alph*)]
  \item National COVID-19 confirmed case counts by region over 24 months,
...
\dpr \textbf{Streaming Payment Operations Dashboard.}
Design a real-time operations dashboard for a mobile payment platform
(10 million transactions/day, peak 500 transactions/second) that shows:
\begin{enumerate}[label=(\alph*)]
  \item Total transaction volume (USD) per minute, last 60 minutes,
        updated every 5 seconds.
  \item Number of active fraud alerts, updated every 30 seconds.
  \item Geographic heatmap of transaction density across a metro area
        (GPS coordinates from merchant terminal locations).
...
Given a simulated dataset of 2 million ride-sharing GPS trip origin points
(longitude, latitude) distributed across five city zones:
\begin{enumerate}[label=(\alph*)]
  ...
  \item Create a 2$\times$2 panel figure (overview in panel 1; one city zone
...
# Five NYC zones: (lon_center, lat_center, sigma, n_points)
zones = [
    (-73.970, 40.785, 0.020, 600_000),  # Upper West Side
    (-73.990, 40.730, 0.015, 500_000),  # Greenwich Village
    (-73.940, 40.760, 0.018, 400_000),  # Midtown East
    (-73.950, 40.820, 0.012, 300_000),  # Harlem
    (-74.010, 40.710, 0.010, 200_000),  # Lower Manhattan
]
...
# Panel 4  --  Detail: Upper West Side zone only
mask = (lons > -73.99) & (lons < -73.95) & (lats > 40.77) & (lats < 40.80)
axes[1,1].hexbin(lons[mask], lats[mask], gridsize=60, cmap="Reds", mincnt=1)
axes[1,1].set_title("(d) Upper West Side detail (Shneiderman: detail)")
...
plt.tight_layout()
plt.savefig("nyc_overplotting.png", dpi=150, bbox_inches="tight")
print("Saved: nyc_overplotting.png")
\end{lstlisting}
...
\texttt{region}, \texttt{category}, and \texttt{revenue\_usd}.
\begin{enumerate}[label=(\alph*)]
  ...
  \item Use \texttt{DataFrame.cube()} to materialise all cuboids for the
        three dimensions \texttt{(order\_date, region, category)}.
  ...
  \item Simulate three Tableau/Superset queries (filter by date range,
        filter by region, group by category) against the cube and
...
\begin{enumerate}[label=(\alph*)]
  \item Thread 1 (``producer''): generates 100 synthetic payment
  ...
  \item Thread 2 (``stream processor''): reads from the queue; aggregates
        over a 10-second tumbling window; computes total USD, transaction
  ...
  \item Thread 3 (``TSDB writer''): writes each window's aggregates
        ...
        Python library) with a \texttt{payment\_metrics} measurement and
        fields \texttt{total\_usd}, \texttt{txn\_count},
        \texttt{fraud\_count}.
...
def stream_processor(write_api):
    ...
    total_usd, txn_count, fraud_count = 0.0, 0, 0

    while True:
        try:
            event = event_queue.get(timeout=0.05)
            total_usd   += event["amount"]
            txn_count   += 1
        ...
        if now - window_start >= WINDOW_SEC:
            ...
            point = (Point("payment_metrics")
                     .field("total_usd",   total_usd)
